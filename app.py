...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="Kepler Multi-Layer Strategy Engine", 
    layout="wide",
    initial_sidebar_state="expanded"
)

# --- GLOBAL VERSION ROUTER ---
st.sidebar.title("🚀 Engine Version Control")
selected_version = st.sidebar.selectbox(
    "Select Model Version",
    ["v21 - Causal Organic SOV (Stable)", "v22 - Experimental Bayesian Saturation"]
)

# =====================================================================
# VERSION 21: CAUSAL ORGANIC SOV ENGINE (STABLE)
# =====================================================================
if selected_version == "v21 - Causal Organic SOV (Stable)":

    st.title("🎯 Kepler Retail Media Portfolio Engine")
    st.subheader("Omni-Channel Multi-Layer Ingestion & Causal Mapping Matrix (v21)")
    ...
    if uploaded_files:
        for file in uploaded_files:
            try:
                ...
                # CRITICAL FIXED ROUTING: Look for Spend and Sales keywords explicitly
    ...
    if df_perf is not None and (df_prod is not None or df_kw is not None or df_brand is not None):
        try:
            ...
            # CRITICAL COLUMN COLLISION FIX: Explicitly ignore NTB headers when looking for total sales dollars
            sales_col = [c for c in df_perf.columns if ('sales' in c or 'revenue' in c) and 'ntb' not in c and 'new' not in c][0]
...
# =====================================================================
# VERSION 22: EXPERIMENTAL BAYESIAN SPEND SATURATION MOCK/WORKSPACE
# =====================================================================
elif selected_version == "v22 - Experimental Bayesian Saturation":
    st.title("📊 Bayesian Spend Saturation Optimization")
    st.subheader("Probabilistic Diminishing Returns & Saturation Mapping (v22)")
    
    st.info("⚡ This sandbox runs custom Markov Chain Monte Carlo (MCMC) simulations to isolate macro budget asymptotes.")

    # Workspace layout showing structural PyMC/Bambi workflow framework
    st.code("""
# Conceptual PyMC/Bambi workflow for Bayesian s-curve causal inference
import pymc as pm
import numpy as np

with pm.Model() as bayesian_s_curve:
    # 1. Priors for the curve parameters
    lower_asymptote = pm.Normal("alpha", mu=0, sigma=10)
    upper_asymptote = pm.Normal("beta", mu=100, sigma=20)
    inflection = pm.Normal("x0", mu=50, sigma=10)
    decay_steepness = pm.HalfNormal("gamma", sigma=1)

    # 2. S-Curve Formula (Diminishing Media Returns based on Spend Volume)
    mu = lower_asymptote + (upper_asymptote - lower_asymptote) / (1 + pm.math.exp(-decay_steepness * (pm.math.log(x / inflection))))
    ...
    # Note: Running this inside web workers requires robust cluster allocation
    # trace = pm.sample(draws=2000, tune=1000, chains=2)
    """, language="python")
    
    st.warning("⚠️ Bayesian fitting blocks stream execution loops during MCMC chains. Run this engine in asynchronous batch processes outside the primary UI loop to preserve stability.")
